...
### Класс Card 
Класс отвечает за визуализацию данных товара в модалке, корзине и в каталоге корзины. Расширяет базовый класс IProduct

Конструктор класса ```(container: HTMLElement, action?: ICardAction)```
...
Наследуется от класса Card и представляет собой карточку, которая отображается на странице. Расширяется класс Card
...
Отвечает за отображение данных превюшки карточек товаров. Расширяет класс CardOnPage
...
Этот класс отвечает за отображение важных элементов на странице, таких как каталог товаров, корзина и счетчик товаров в корзине. Он расширяет базовый класс Component и реализует интерфейс IPage. В этом классе есть поля, которые связаны с HTML-разметкой, и методы, которые заполняют разметку данными. Кроме того, есть методы для открытия и закрытия модального окна, которые также влияют на прокрутку страницы.

Конструктор класса: ```(container: HTMLElement, protected events: IEvents)```

#### Поля класса
- ```hdrBasketCounter``` -  хранит рамзетку счетчика товаров в корзине

- ```gallery``` - хранит разметку галлереи товаров

- ```pageWrapper``` - разметка обёртки страницы
